...
if ($_SERVER ["REQUEST_METHOD"] == "POST") {	
	try {			
			...
			$stmtLoanType = $user_login->runQuery ( "SELECT loantypeid,loantyperate,loantypename,loanfinerate,loanminimumduration,loanmaximumduration,loanminimumage
													,loanmaximumage,loanprocessingfee,securitycoverpercent FROM loantype WHERE loantypeid =:loantypeid" );
			...
			//Insert Payment Schedule and event into database
			for($i=1; $i <=$loanDuration; $i++){
				...
				$insertEvent->execute(array($newLoanId,$eventTitle,1,$paymentDate,$eventStartTime,$paymentDate,$eventEndTime,$eventDiscription,$eventOwner,$eventLocation));
				...
				$insertPaymentSchedule->execute(array($newLoanId,$paymentDate,$paymentPrinciple,$paymentInterest,$totalPaymentPerMonth));
			}
			//End insert Payment Schedule and event into database
			
			//Set approval initial parameters
			$approvaltype = 1; //initial stage approval before issuing loan
			$comments = "New application add security,security photo, contract and other attachments";
			$approvaldate = null;
			$approvedby = null;
			$loanid = $newLoanId;
			//Insert approval parameters 
			$insertLoanApproval = $user_login->runQuery("INSERT INTO loanapproval (loanid,approvaltype,comments,approvaldate,approvalby) VALUES (?,?,?,?,?)");
			$insertLoanApproval->execute(array($loanid,$approvaltype,$comments,$approvaldate,$approvedby));
